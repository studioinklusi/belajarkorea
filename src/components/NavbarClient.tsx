...
export default function NavbarClient({ user, profile, isAdmin, isLandingPage, signoutAction }: NavbarClientProps) {
  ...
  if (pathname) {
    ...
    else if (pathname.startsWith('/tebak-hangul')) activePage = 'tebak-hangul'
                    ...
                    <div className="relative">
                      <button
                        ...
                        className={`flex items-center gap-1 px-4 py-2.5 rounded-full text-sm font-bold transition-colors cursor-pointer ${
                          isLearningOpen || ['courses', 'stories', 'flashcards', 'ai-buddy', 'quiz-games', 'tebak-hangul'].includes(activePage)
                      ...
                      {isLearningOpen && (
                        ...
                        >
                          ...
                          <Link 
                            href="/tebak-hangul" 
                            onClick={() => setIsLearningOpen(false)}
                            className="flex items-center gap-3.5 p-3 rounded-2xl hover:bg-amber-50/50 hover:backdrop-blur-md transition-all duration-300 transform hover:scale-[1.02] group"
                          >
                            <div className="w-10 h-10 rounded-xl bg-gradient-to-tr from-amber-100 via-yellow-50 to-orange-100 text-amber-600 flex items-center justify-center font-bold shrink-0 shadow-xs border border-amber-200/30">
                              <FaPuzzlePiece className="w-5 h-5" />
                            </div>
                            <div>
                              <p className="text-sm font-extrabold text-gray-900 group-hover:text-amber-700 transition-colors">Tebak Hangul</p>
                              <p className="text-[11px] text-gray-400 font-semibold leading-none mt-0.5">Quiz cepat mengenal huruf Korea</p>
                            </div>
                          </Link>
                        </div>
                      )}
        ...
        <div className="flex justify-around items-center h-16 px-2">
          ...
          {user ? (
            <button 
              ...
              className={`flex flex-col items-center justify-center w-full h-full space-y-1 ${
                ['courses', 'stories', 'flashcards', 'ai-buddy', 'quiz-games', 'tebak-hangul'].includes(activePage) ? 'text-violet-600' : 'text-gray-400 hover:text-gray-600'
              }`}
            >
              <FaBookOpen className={`w-5 h-5 transition-transform ${['courses', 'stories', 'flashcards', 'ai-buddy', 'quiz-games', 'tebak-hangul'].includes(activePage) ? 'scale-110' : ''}`} />
          ...
          <div className="fixed bottom-0 left-0 right-0 bg-white rounded-t-3xl shadow-[0_-10px_40px_rgba(0,0,0,0.12)] pt-4 pb-safe z-[61] animate-slide-up flex flex-col md:hidden">
            ...
            <div className="flex flex-col gap-2 px-4 pb-6 mt-3 max-h-[70vh] overflow-y-auto">
              ...
              <Link 
                href="/tebak-hangul" 
                className={`flex items-center gap-4 p-3.5 rounded-2xl transition-all border ${
                  activePage === 'tebak-hangul' 
                    ? 'bg-amber-50/70 border-amber-100' 
                    : 'hover:bg-amber-50/30 border-transparent hover:scale-[1.01] transform duration-200'
                }`}
              >
                <div className="w-11 h-11 rounded-xl bg-gradient-to-tr from-amber-100 via-yellow-50 to-orange-100 text-amber-600 flex items-center justify-center font-bold shrink-0 shadow-xs border border-amber-200/30">
                  <FaPuzzlePiece className="w-5.5 h-5.5" />
                </div>
                <div className="flex-1">
                  <p className="text-sm font-extrabold text-gray-900">Tebak Hangul</p>
                  <p className="text-[11px] text-gray-400 font-semibold leading-tight mt-0.5">Quiz cepat mengenal huruf Korea</p>
                </div>
              </Link>
            </div>
          </div>
