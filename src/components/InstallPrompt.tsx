...
export default function InstallPrompt() {
  ...
  useEffect(() => {
    ...
    // Show banner on mobile after 2 seconds (unless dismissed recently)
  ...
  // Listen for manual install trigger from other components (e.g. MobileMenu)
  // This listener is ALWAYS active regardless of banner/install state
  useEffect(() => {
  ...
  // iOS/Android guide modal - always render if triggered (even if banner is hidden)
  const guideModal = showIOSGuide ? (
  ...
  return (
    <>
      {/* Install Banner - visible on mobile only, when not dismissed */}
      ...
      {/* Guide Modal - always renderable, triggered by manual install or fallback */}
      {guideModal}
    </>
  )
}
