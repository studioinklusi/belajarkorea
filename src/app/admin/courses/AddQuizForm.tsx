...
import { FaXmark, FaSpinner, FaCircleCheck, FaPlus, FaTrash } from 'react-icons/fa6'

export default function AddQuizForm({ lessonId, lessonName, onClose }: { lessonId: string; lessonName: string; onClose: () => void }) {
    ...
    return (
      <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
        <div className="bg-white rounded-3xl p-10 max-w-md w-full text-center shadow-2xl animate-fade-in-up">
          ...
          <h3 className="text-2xl font-extrabold text-gray-900 mb-2">Soal Berhasil Ditambahkan!</h3>
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-3xl p-8 max-w-lg w-full shadow-2xl relative max-h-[90vh] overflow-y-auto">
        ...
        <h2 className="text-2xl font-extrabold text-gray-900 mb-1">Tambah Soal Quiz</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          >
            {loading ? <><FaSpinner className="animate-spin" /> Menyimpan...</> : <><FaPlus /> Tambahkan Soal</>}
          </button>
        </form>
      </div>
    </div>
